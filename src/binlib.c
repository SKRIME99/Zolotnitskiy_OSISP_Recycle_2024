...
void put_file_to_bin_with_checks(char *old_path_name, char *new_path)
{
    ...
    log_new_file(old_path_name, tmp_path);
}
...
void delete_file_forever(const char *filename)
{
    ...
    while(!feof(f))
    {
        ...
        println();
        delete_line_from_binlog_file(dest);
        fclose(f);
        return;
    }
    fclose(f);
...
void restore_file_from_bin(const char *filename)
{
    FILE *f = fopen(trash_log_path, "r+");
    if(!f)
    {
...
void delete_line_from_binlog_file(const char *dest)
{
    int fd = open(trash_log_path, O_RDWR);
    if(!fd)
    {
...
void log_new_file(const char *path_name, const char *new_path)
{
    time_t rawtime;
    struct tm * timeinfo;
    time (&rawtime);
